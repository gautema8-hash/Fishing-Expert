"""
Final BOSS fix v3: hard cutoff for bottom water reflections.
"""
import os
...
# Below 80% height: only keep very bright, strong golden (dragon body)
# Vectorized
cutoff = int(h * 0.80)
bottom = np.zeros_like(alpha, dtype=bool)
bottom[cutoff:, :] = True
# In bottom region, remove pixels that are dim or not strongly warm
bottom_remove = bottom & ((warmth < 40) | (brightness < 130))
alpha[bottom_remove] = 0

# Edge feather for warmth boundary
